...
/**
 * This class implements a partial HTML parser as well as a parse tree node generated by the parser
 * (and thus is a candidate for refactoring).
 * <p>
 ...
 * structure.
 * <p>
 * Each instance of this class also represents a node of the parse tree, where {@code more} points
 * to the next sibling node and {@code parts} points to the first child node.
 * <p>
 * The {@code leader} and {@code trailer} members contain unparsed text preceding or following the
 * current element.
 * <p>
 * {@code tag} is the opening tag of the current element, including the angular brackets and any
 * attributes.
 * <p>
...
public class Parse {
    ...
    /**
     * Creates a parse tree from the given text by parsing it completely, creating a hierarchy of
     * child nodes according to the given list of tags.
     * 
     * @param text text to be parsed
     * @param tags Tags corresponding to the tree levels. All nodes at level {@code i} will have a
     *        tag matching {@code tags[i]} (the virtual root node of the tree can be regarded as
     *        having level -1).
     * @throws ParseException
     */
    ...
    /**
     * Returns the number of sibling nodes following this node. For the first child node of a given
     * parent, this is equal to the size of the child list.
     * 
    ...
    /**
     * Converts the body of this node to plain text by normalizing line breaks and unescaping HTML
     * entities.
     */
    public String text() {
    ...
    /**
     * Inserts the given text into the tag of this node, before the closing bracket. (Used to add
     * attributes to a tag.)
     * 
